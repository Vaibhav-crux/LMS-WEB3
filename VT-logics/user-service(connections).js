...
class UserService {
 async register(userId, courseId, mappedCourseId) {
    try {
      dbConnect();
      const [findCourse, findUser, findConnection] = await Promise.all([
        Course.findOne({ _id: courseId }),
        User.findOne({ userId: userId }),
        Connection.findOne({ course_id: courseId, mapped_course_id: mappedCourseId }),
      ]);
      ...
      } else if (!findConnection) {
        throw new Error("Connection not found with course ID: " + courseId + " and mapped course ID: " + mappedCourseId);
      }
      ...
      if (existingProgress) {
        ...
        if (!lastProgress.isCourseLocked && lastProgress.isCourseCompleted && lastProgress.totalCourseScore >= 75) {
          ...
          // Your existing logic to create a new object in user_progress goes here
          // For example:
